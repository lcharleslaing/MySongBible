...
const emptyApp: HomeAppRecord = { id: "", label: "", description: "", path: "/", badge: "App" };
...
export function HomePageSettingsCard({ disabled = false }: { disabled?: boolean }) {
  ...
  const save = async () => {
    const normalized = { ...appDraft, id: appDraft.id || slugify(appDraft.label), label: appDraft.label.trim(), description: appDraft.description.trim(), badge: appDraft.badge.trim() || "App" };
  ...
  return <>
    ...
    {isOpen ? <dialog className="modal modal-open" aria-labelledby="home-page-editor-title"><div className="modal-box max-w-3xl">
      ...
      {step === 1 ? <div className="grid gap-4 md:grid-cols-2"><label className="form-control gap-2"><span className="label-text font-medium">App name</span><input autoFocus className="input input-bordered" value={appDraft.label} onChange={(event) => setAppDraft({ ...appDraft, label: event.target.value, id: editingId ? appDraft.id : slugify(event.target.value) })} /></label><label className="form-control gap-2"><span className="label-text font-medium">Badge</span><input className="input input-bordered" value={appDraft.badge} onChange={(event) => setAppDraft({ ...appDraft, badge: event.target.value })} /></label><label className="form-control gap-2"><span className="label-text font-medium">Route</span><input className="input input-bordered" value={appDraft.path} onChange={(event) => setAppDraft({ ...appDraft, path: event.target.value })} placeholder="/my-app" /></label><label className="form-control gap-2"><span className="label-text font-medium">App ID</span><input className="input input-bordered" value={appDraft.id} onChange={(event) => setAppDraft({ ...appDraft, id: slugify(event.target.value) })} /></label><label className="form-control gap-2 md:col-span-2"><span className="label-text font-medium">Description</span><textarea className="textarea textarea-bordered min-h-24" value={appDraft.description} onChange={(event) => setAppDraft({ ...appDraft, description: event.target.value })} /></label></div> : null}
      {step === 2 ? <div className="space-y-4"><div className="card border border-base-300 bg-base-200/50"><div className="card-body"><span className="badge badge-primary badge-outline w-fit">{appDraft.badge || "App"}</span><h3 className="card-title">{appDraft.label}</h3><p className="text-sm text-base-content/70">{appDraft.description}</p><p className="text-xs text-base-content/50">Dashboard and left menu route: {appDraft.path}</p></div></div><p className="text-sm text-base-content/60">Saving updates both the home dashboard and the left navigation menu.</p></div> : null}
